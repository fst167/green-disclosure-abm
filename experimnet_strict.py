...
def experiment_task(v, lambdas_factor, lambdas_step, ka, ka_step, ks, ks_step, num_dirty, num_low_green, num_high_green, beta, total_subsidy):
    # Adjust lambdas
    f = lambdas_factor + v * lambdas_step  # Calculate lambdas_factor based on v
    lambdas = adjust_lambdas(f)
    
    inner_results = []
    pi_results = []  # For collecting pi data
    for i in range(10):
        inner_inner_results = []    
        ka = [ka + j * ka_step for j in range(10)][i]
        for p in range(10):
          ks = [ks + j * ks_step for j in range(10)][p]
          try:
              ...
              # Save pi value
              pi_results.append((v, i, p, pi))  # Record v, i, p and pi
              ...
              inner_inner_results.append(agent_set)
          except Exception as e:
              print(f"Error at lambda['dirty'] = {f}, ka = {ka}: {e}")
              inner_inner_results.append("error")
        inner_results.append(inner_inner_results)
    
    return v, inner_results, pi_results  # Return pi_results
...
if __name__ == "__main__":
    ...
    pi_data = []  # For saving all pi data
    ...
    results = [pool.apply_async(experiment_task, args=(v, lambdas_factor, lambdas_step, ka, ka_step, ks, ks_step, num_dirty, num_low_green, num_high_green, beta, total_subsidy)) for v in range(10)]

    # Get results from each process and store in experiment_results according to outer loop order
    for res in results:
        v, inner_results, pi_results = res.get()  # Get v, inner_results and pi_results returned by multiprocessing
        experiment_results[v] = inner_results  # Save results in order of v
        pi_data.extend(pi_results)  # Collect pi results
    ...
    file_path = '/scratch/users/Green Disclosure/result/experiment_results.txt'
    with open(file_path, 'w') as file:
      for result in experiment_results:
          result_str = [str(item) for item in result]  # Convert each element to string
          file.write(", ".join(result_str) + "\n")

    # Save pi data
    pi_file_path = '/scratch/users/Green Disclosure/result/pi_results.txt'
    with open(pi_file_path, 'w') as pi_file:
      for v, i, p, pi in pi_data:
          pi_file.write(f"v={v}, i={i}, p={p}, pi={pi}\n")
